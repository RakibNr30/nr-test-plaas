    ...
    */
    public function setUp(): void
    {
        $this->showCheckBox()
            ...
            ->showPerPage()
            ->showSearchInput()
            ->showExportOption('User-' . time(), ['excel', 'csv']);
    }
    ...
    public function addColumns(): ?PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('Action', function (User $data) {
                return view('livewire.user.action', compact('data'));
            });
    }
    ...
    public function actions(): array
    {
       return [
           Button::add('show')
               ->caption('Show')
               ->target('')
               ->class('btn btn-success btn-sm cursor-pointer text-white px-3 py-2.5 m-1 rounded text-sm')
               ->route('user.show', ['user' => 'id']),

           Button::add('edit')
               ->caption('Edit')
               ->target('')
               ->class('btn btn-primary btn-sm  cursor-pointer text-white px-3 py-2.5 m-1 rounded text-sm')
               ->route('user.edit', ['user' => 'id']),

           Button::add('destroy')
               ->caption('Delete')
               ->target('')
               ->class('btn btn-danger btn-sm  cursor-pointer text-white px-3 py-2 m-1 rounded text-sm')
